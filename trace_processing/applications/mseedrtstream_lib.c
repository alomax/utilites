/***************************************************************************
 * mseedrtstream_lib.c - Mini-SEED data sorted to simulate a real-time stream.
 *
...
//AJL static DLCP *dlconn = 0;

// AJL added following from libdali/libdali.h
...
// AJL added following function
/***********************************************************************//**
 * @brief returns list of input files
 *
 ***************************************************************************/

Filelink* getFilelist() /* List of input files */
{
    return(filelist);
}

// AJL added following function from libdali/portable.c
/***********************************************************************//**
...
mainXXX(int argc, char **argv) {
    ...
    //if (processparam_mseedrtstream(argc, argv) < 0)
    //    return 1;
...
readfiles(RecordMap *recmap) {
    ...
    //AJL - not needed for application in miniseed_process
    //AJL setofilelimit(totalfiles + 20);
...
writerecords(RecordMap *recmap) {
    ...
    while (rec && errflag != 1) {
        ...
        if (rec->reclen > (int) sizeof (recordbuf)) {
...
sendrecord(char *recbuf, Record *rec) {
    ...
    /*AJL if (dl_write(dlconn, recbuf, rec->reclen, streamid,
        ...
        while (p) {
            ...
            while (psize > 0 || (qsize > 0 && q)) {
                ...
                if (psize == 0) { /* p is empty; e must come from q. */
                    ...
                    p = p->next;
                    psize--;
                } else if (recordcmp(p, q) <= 0) { /* First element of p is lower (or same), e must come from p. */
                    e = p;
                    p = p->next;
                    psize--;
...
recordcmp(Record *rec1, Record *rec2) {
    ...
    // AJL if (rec1->endtime > rec2->endtime) {
...
//AJL static char *dladdress = 0;
static char *tptr;

/***************************************************************************
 * processparam_flag_mseedrtstream():
 * Process a flag command line parameter.
 *
 * Returns 0 on success, and -1 on failure
 ***************************************************************************/
int
processparam_flag_mseedrtstream(int argcount, char **argvec, int *potind) {

    /* AJL if (strcmp(argvec[*potind], "-V") == 0) {
        ms_log(1, "%s version: %s\n", PACKAGE, VERSION);
        exit(0);
    } else if (strcmp(argvec[*potind], "-h") == 0) {
        usage();
        exit(0);
    } else */ if (strncmp(argvec[*potind], "-v", 2) == 0) {
        verbose += strspn(&argvec[*potind][1], "v");
    } else if (strcmp(argvec[*potind], "-sum") == 0) {
        basicsum = 1;
    } else if (strcmp(argvec[*potind], "-ts") == 0) {
        starttime = ms_seedtimestr2hptime(getoptval(argcount, argvec, *potind++));
        if (starttime == HPTERROR)
            return -1;
    } else if (strcmp(argvec[*potind], "-te") == 0) {
        endtime = ms_seedtimestr2hptime(getoptval(argcount, argvec, *potind++));
        if (endtime == HPTERROR)
            return -1;
    } else if (strcmp(argvec[*potind], "-M") == 0) {
        matchpattern = strdup(getoptval(argcount, argvec, *potind++));
    } else if (strcmp(argvec[*potind], "-R") == 0) {
        rejectpattern = strdup(getoptval(argcount, argvec, *potind++));
    } else if (strcmp(argvec[*potind], "-sd") == 0) {
        streamdelay = 1;
    } else if (strcmp(argvec[*potind], "-df") == 0) {
        streamdelay = 1;
        delayfactor = strtod(getoptval(argcount, argvec, *potind++), NULL);
        //} else if (strcmp(argvec[*potind], "-o") == 0) {
        //    outputfile = getoptval(argcount, argvec, *potind++);
        //} else if (strcmp(argvec[*potind], "-dl") == 0) {
        //    dladdress = getoptval(argcount, argvec, *potind++);
    } else {
        return(-1);
    }
 ...
 ***************************************************************************/
int
processparam_files_mseedrtstream(int argcount, char **argvec, int optind) {
 ...
 ***************************************************************************/
int
processparam_init_mseedrtstream() {
    ...
    /*AJL if (dladdress && !(dlconn = dl_newdlcp(dladdress, argvec[0]))) {
 ...
 ***************************************************************************/
void
usage_mseedrtstream(void) {
    //AJL fprintf(stderr, "%s - Create simulated real-time stream of Mini-SEED: %s\n\n", PACKAGE, VERSION);
    //AJL fprintf(stderr, "Usage: %s [options] file1 [file2] [file3] ...\n\n", PACKAGE);
    fprintf(stderr,
            " ## mseedrtstream_lib Options ##\n"
